...
async function getProfileAndPosts(userid: number) {
  try {
    const profileResponse = await apiClient.get(`/users/profile/${userid}`);
    const postsResponse = await apiClient.get(`/posts/${userid}`);
...
const UserProfile = async ({ params }: { params: { userid: number } }) => {
  const profileAndPostsData = await getProfileAndPosts(params.userid);
  const { profile, posts } = profileAndPostsData!;
  return (
